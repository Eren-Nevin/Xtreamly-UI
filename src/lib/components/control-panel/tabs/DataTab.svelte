<script lang="ts">
	import type { AuthData, DynamicAccess } from '$lib/ProxyAccount';
	import type { Writable } from 'svelte/store';
	import DataControlRow from '../DataControlRow.svelte';

	let advancedAccessControl = false;
	export let dynamicAccess: Writable<DynamicAccess>;
	export let authDatas: Writable<AuthData[]>;
</script>
...
<div class="w-full flex flex-col items-center px-2">
	<div class="flex flex-row space-x-2 items-center">
		...
		<button class="btn btn-cirlce btn-warning btn-sm">Add</button>
	</div>
